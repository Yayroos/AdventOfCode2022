let input = `DMwrszrfMzSSCpLpfCCn
...
input.forEach(line => {
    let first = Array.from(line.substring(0, line.length/2));
    let second = Array.from(line.substring(line.length/2));
...
duplicates.forEach(item => {
    if(isUpperCase(item)){
        sum += (item.codePointAt(0) - 64) + 26;
    } else {
        sum += (item.codePointAt(0) - 96);
    }
